...
const deliveryInfoSchema = new mongoose.Schema(
  {
    ...
    baseCharges: {
      type: Number,
      default: 0,
      min: [0, "Base delivery charges cannot be negative"],
    },
    distance: {
      type: Number,
      default: null,
...
orderSchema.pre("save", async function (next) {
  try {
    ...
    // ✅ ADD: Calculate correct total with delivery charges
    const itemsTotal = this.items.reduce(
      (sum, item) => sum + item.totalPrice,
      0
    );
    const calculatedTotal =
      itemsTotal - this.discount + this.deliveryCharges + this.tax;

    // Update total if significantly different (allow for small floating point differences)
    ...
    this.items.forEach((item, index) => {
      ...
      if (Math.abs(item.totalPrice - calculatedItemTotal) > 0.01) {
        this.items[index].totalPrice = calculatedItemTotal;
      }
    });
    ...
    case "confirmed":
      ...
      if (additionalData.paymentInfo) {
        ...
        this.payment.paymentDate =
          additionalData.paymentInfo.paymentDate || new Date();
...
orderSchema.statics.isTransactionIdUsed = async function (
  transactionId,
  excludeOrderId = null
) {
  ...
  const query = {
    ...
    "payment.paymentStatus": {
      $in: ["pending_verification", "verified", "completed"],
    },
  };
...
orderSchema.index({ "deliveryInfo.isFreeDelivery": 1 });
