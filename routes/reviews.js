// routes/reviews.js - Complete String-based MongoDB Integration with Edit/Delete
...
const getAuthenticatedUser = (req) => {
  try {
    ...
    if (req.body && req.body.currentUser && req.body.currentUser._id) {
      return {
...
router.get('/product/:productId', async (req, res) => {
  try {
    ...
    const reviewsWithVoteStatus = reviews.map(review => ({
      ...
      userId: {
        _id: review.userId, // Include the actual userId for comparison
...
router.post('/', async (req, res) => {
  try {
    const { productId, rating, title, comment, images, currentUser, isEditing, reviewId } = req.body;
    ...
    if (images && Array.isArray(images)) {
      processedImages = images.slice(0, 5);
...
router.put('/:reviewId', async (req, res) => {
  try {
    ...
    const { rating, title, comment, images, currentUser, productVariant } = req.body;
    ...
    if (!rating || !title || !comment) {
      return res.status(400).json({
        success: false,
        message: 'Missing required fields: rating, title, comment'
      });
    }
    ...
    if (images && Array.isArray(images)) {
      processedImages = images.slice(0, 5);
      console.log(`📸 Processing ${processedImages.length} images for updated review`);
    }
    ...
    console.log(`✅ Review updated in MongoDB by: ${user.name} (${user.email})`);
    ...
    res.json({
      ...
      review: {
        ...
        hasUserVoted: updatedReview.helpfulBy.includes(user._id),
        userId: {
          _id: user._id,
          name: user.name,
          email: user.email,
...
router.delete('/:reviewId', async (req, res) => {
  try {
    ...
    console.log(`🗑️ Review deleted by: ${user.name} (${user.email})`);
...
router.get('/health', async (req, res) => {
  try {
    ...
    res.json({
      status: 'OK',
      service: 'Reviews API (Complete String-based MongoDB)',
      totalReviews,
...
console.log('📋 Complete Reviews router configured with String-based MongoDB integration + Edit/Delete');
